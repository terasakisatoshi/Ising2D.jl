...
@inline function Base.getindex(arr::CircularMatrix, i::Int)
    ...
    @inbounds getindex(arr.data, _resolve_index(r, i))
end

@inline function Base.getindex(arr::CircularMatrix{T}, I::Vararg{Int,2}) where {T}
    r1 = axes(arr.data, 1)
    r2 = axes(arr.data, 2)
    i1 = @inbounds _resolve_index(r1, I[1])
    i2 = @inbounds _resolve_index(r2, I[2]) 
    @inbounds getindex(arr.data, i1, i2)
end

@inline function Base.setindex!(arr::CircularMatrix, v, i::Int)
    r = eachindex(IndexLinear(), arr.data)
    setindex!(arr.data, v, _resolve_index(r, i))
end

@inline function Base.setindex!(arr::CircularMatrix{T}, v, I::Vararg{Int,2}) where {T}
    r1 = axes(arr.data, 1)
    r2 = axes(arr.data, 2)
    i1 = @inbounds _resolve_index(r1, I[1])
    i2 = @inbounds _resolve_index(r2, I[2]) 
    @inbounds setindex!(arr.data, v, i1, i2)
end
